...
export default function WeeklyOpenAgeTrendCard({ weeklyOpenAgeTrend, mode }) {
  ...
  return (
    <>
      {PRIORITY_ORDER.map((bucket) => (
        ...
        >
          <WeeklyPriorityTrendChart points={points} bucket={bucket} color={priorityColor(bucket, mode)} />
        </MetricCard>
      ))}
    </>
  );
